$(document).ready(function() {
    ...
    $('#datepicker-example2_lr1').Zebra_DatePicker({
		format: 'd-M-y',
        direction: 1    // boolean true would've made the date picker future only
                        // but starting from today, rather than tomorrow
    });
	
	$('#datepicker-example2_lr').Zebra_DatePicker({
		format: 'd-M-Y', 
		direction: false // 01-07-2016
	});
    ...
    $('#datepicker-example3').Zebra_DatePicker({
		format: 'd-M-y',
        direction: true,
       // disabled_dates: ['* * * 0,6']   // all days, all monts, all years as long // as the weekday is 0 or 6 (Sunday or Saturday)
    });
	...
	for(var idp = 0; idp < 10000; idp+=1) 
	{
		$('#datepicker-example3_i_'+idp).Zebra_DatePicker({
			format: 'd-M-y',
			direction: true,
		});
	}
	...
	for(var idp = 100001; idp < 111001; idp+=1) 
	{
		$('#datepicker-example3_i_'+idp).Zebra_DatePicker({
			format: 'd-M-y',
			direction: true,
	...
	for(var idp = 0; idp < 10000; idp+=1) 
	{
		$('#datepicker-example3_j_'+idp).Zebra_DatePicker({
			format: 'd-M-y',
			direction: true,
		});
	}
	...
	for(var idp = 0; idp < 1000; idp+=1) 
	{
		$('#datepicker-example3_'+idp).Zebra_DatePicker({
			format: 'd-M-y',
			direction: true,
			//direction: ['2015-04-20', '2015-04-30']
		});
	}
	
	for(var idp = 0; idp < 1000; idp+=1) 
	{
		$('#datepicker-example3to_'+idp).Zebra_DatePicker({
			format: 'd-M-y',
			direction: true,
			//direction: ['2015-04-20', '2015-04-30']
		});
	...
	$('#datepicker-example77-start').Zebra_DatePicker({
		format: 'd-M-Y',
      //  direction: true,
        pair: $('#datepicker-example77-end')
    });

    $('#datepicker-example77-end').Zebra_DatePicker({
		format: 'd-M-Y',
        direction: 1
    });
